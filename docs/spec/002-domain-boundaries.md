...
**Operational state** is FeatureForge's own mutable bookkeeping: which projects
exist, which feature cards exist, what a card is called today. It is
product-owned, mutable in place, and has no engineering meaning. It is never
...
values. It is insert-only and is never edited.
...
| Property | Value |
|---|---|
| Identity | Product-owned, opaque, generated by FeatureForge |
| Mutable state | Yes — name, description |
| References PEOS | No |
...
|---|---|
| Identity | Product-owned, opaque, generated by FeatureForge |
| Mutable state | Yes — title, summary, and the project it belongs to |
...
Three further things are FeatureForge-owned but are neither operational entities
